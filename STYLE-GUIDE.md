...
- Most fireflies drift locally; **one or two** may travel the full width of the field in
  long, mostly-horizontal passes (swooping around at the edges, with occasional big slow
  loops) so a child can track one crossing the screen — still within the drift speed cap,
  the reach coming from heading, not speed. The on-screen count stays within a calm
  floor/ceiling (4–6): the sky is never empty and never a swarm.
...
- A tapped firefly holds the surprised front pose, then makes **one** slow, eased turn
  (front → side → back → front, from the character sheet's turnaround) on its float to the
  jar — a single rotation, never a spin-cycle, and the glow stays steady through it (no
  strobe). Idle and sleepy fireflies never spin.
